        ...
        <div class="card-body d-flex justify-content-start align-items-center">
            <div class=" card table-responsive col-12 font-bold p-3">
                <div class="d-flex mb-2 row">
                    ...
                    <div class="" style="max-width: 10rem; margin-left: 2rem;">
                        @if(!$dateFrom && !$dateTo)
                            <button class="btn btn-secondary d-none" data-bs-toggle="modal" data-bs-target="#generateModal" >
                                Generate Payslip
                            </button>
                        @else
                            <button class="btn btn-secondary " data-bs-toggle="modal" data-bs-target="#generateModal" >
                                Generate Payslip
                            </button>
                        @endif     
                    </div>
                ...
                <table class="table bordered bg-white table-hover ">
                    ...
                    @foreach($getEmployee as $employee)
                        ...
                        <td class="col-1 border">{{ array_key_exists($employee->employee_id, $totalDays) ? $totalDays[$employee->employee_id] : '0'}}</td>
                        <td class="col-1 border">{{ number_format(array_key_exists($employee->employee_id, $totalOvertime) ? $totalOvertime[$employee->employee_id] : '0',2)}}</td>
                        <td class="col-1 border" >{{ $employee->daily_rate }}</td>
                        @php
                            $OT = array_key_exists($employee->employee_id, $totalOvertime) ? $totalOvertime[$employee->employee_id] : 0;
                            $totalOT = number_format(($employee->daily_rate /8)* $OT,2);
                            $totalGrossWithOT = array_key_exists($employee->employee_id, $totalDays) ? $totalDays[$employee->employee_id] * $employee->daily_rate + $totalOT : '0';
                            $deductions = number_format(array_key_exists($employee->employee_id, $totalCashAdvance) ? $totalCashAdvance[$employee->employee_id] : '0',2);
                            $finalPay = number_format($totalGrossWithOT - $deductions,2);
                        @endphp
                        {{-- gross total --}}
                        <td class="col-1 border" >{{ number_format(array_key_exists($employee->employee_id, $totalDays) ? $totalDays[$employee->employee_id] * $employee->daily_rate + $totalOT : '0',2)}}</td>
                        {{-- deductions --}}
                        <td class="col-1 border">{{ number_format(array_key_exists($employee->employee_id, $totalCashAdvance) ? $totalCashAdvance[$employee->employee_id] : '0',2)}}</td>
                        {{-- net total --}}
                        ...
                        {{-- <td class="col-1 border">
                            <div class="d-flex justify-content-center align-items-center " style="height: 100%">
                                ...
                                <a href="#">
                                    <span class="bi bi-eye point" style="font-size: 2rem; " data-toggle="tooltip"
                                        title="View Payslip"></span>
                                </a>
                            </div>
                        </td> --}}
                    </tr>
            ...
            <div class="modal fade text-left w-100" id="generateModal" tabindex="-1"
                role="dialog" aria-labelledby="salarySummary" aria-hidden="true">
